...
function ProductDescription(){
    ...
    useEffect(()=>{
        const getindividualproduct = async () => {
            try {
                const res = await axios.get('http://localhost:8000/api/v1/product/getProductById/'+ id);
                if(res.data.success){
    ...
    return(
        <>
            <div className='productdescriptioncontainer'>
                ...
                <Thirdpart withexchange={itemdetails.withexchange} withoutexchange={itemdetails.withoutexchange} delivery={itemdetails.delivery} availibility={itemdetails.availibility}  
